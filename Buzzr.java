...
/*      Dimension x = getSize();
    ...
    public void updateCircuit(Graphics realg) {
        ...
        if (!stoppedCheck.getState() && circuitMatrix != null) {
            ...
            long delay = 1_000 / 50 - (System.currentTimeMillis() - lastFrameTime);
    ...
    void runCircuit() {
        ...
        if (1_000 >= steprate * (tm - lastIterTime)) {
            return;
        }
        for (iter = 1;; iter++) {
            int i, j, k, subiter;
            ...
            lit = tm;
            if (iter * 1_000 >= steprate * (tm - lastIterTime) || (tm - lastFrameTime > 500)) {
                break;
            }
        }
        lastIterTime = lit;
...
//		 ImportExportDialog.Action.EXPORT);
    ...
    public void mouseMoved(MouseEvent e) {
        ...
        int bestDist = 100_000;
        int bestArea = 100_000;
        for (i = 0; i != elmList.size(); i++) {
            CircuitElm ce = getElm(i);
    ...
    CircuitElm constructElement(Class c, int x0, int y0) {// find element class
    ...
    void pushUndo() {
        ...
        if (undoStack.size() > 0
                && s.compareTo(undoStack.get(undoStack.size() - 1)) == 0) {
            return;
        }
        undoStack.add(s);
        enableUndoRedo();
